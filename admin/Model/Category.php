...
class Category {
    ...
    public function addCategory($title){
        ...
        $isExists = $this->checkDuplicate($title);
        if(count($isExists) == 0){
            $pdo = $this->getPdo();
    ...
    public function editCategory($title, $catId){

        $isExists = $this->checkDuplicate($title);
        if(count($isExists) == 0){
            $pdo = $this->getPdo();
            $sql = 'UPDATE `category` SET `title` = "' . $title . '" WHERE `cat_id` = ' . $catId . '';
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            return true;
        }else{
...
// Total data set length
        ...
        while ( $aRow = $rResult->fetch_assoc() ) {
            ...
            $row[] = '<i style="cursor:pointer;" data-action="edit" data-id="'.$catId.'" data-title="'. $title .'"  onClick="pushData(this);" data-toggle="modal"  href="#modalEditCategory" class="fa fa-edit editTooltip"></i>
                <i style="cursor:pointer;" data-action="delete" data-id="'.$catId.'" data-title="'. $title .'"  onClick="pushData(this);" data-toggle="modal"  href="#modalDeleteCategory" class="fa fa-times deleteTooltip"></i>';
            $output['aaData'][] = $row;

...
class Products {
    ...
    public function addProduct($catId, $supId, $stockId, $productName, $description, $originalPrice, $sellingPrice, $quantity, $profit, $brandName, $dateArrival, $image){
    ...
    public function editProduct($productId, $catId, $supId, $stockId, $productName, $description, $originalPrice, $sellingPrice, $quantity, $profit, $brandName, $dateArrival, $image){
    ...
    public function getProductLatest(){
        $pdo = $this->getPdo();
        $sql = 'SELECT * FROM `products` WHERE `featured` = 0 ORDER BY `date_arrival` DESC LIMIT 6';
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $content = array();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    public function getProductByCategory($catId, $sort, $direction){
        $pdo = $this->getPdo();
        $sql = 'SELECT * FROM `products` WHERE `cat_id` = "' . $catId . '" ORDER BY `' . $sort . '`' . $direction;
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $content = array();
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    public function getProducts(){
        $aColumns = array('product_id', 'cat_id', 'brand_id', 'sup_id', 'stock_id', 'product_name', 'description', 'date_arrival', 'original_price', 'selling_price', 'profit', 'borrowed_item', 'product_qty', 'no_item_sold', 'total_profit', 'image_name');
